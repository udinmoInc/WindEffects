...
void SwapchainManager::CreateSwapchain() {
    ...
    // Prefer platform currentExtent when the surface locks it (Win32).
    VkExtent2D extent{};
    ...
    WE_VALIDATE_INIT(extent.width > 0 && extent.height > 0, "SwapchainManager", "Swapchain extent is zero.");
    ...
    const uint32_t graphicsFamily = m_Config.deviceContext->GetGraphicsQueueFamily();
    const uint32_t queueFamilies[] = { graphicsFamily, m_PresentQueueFamily };
    const bool concurrent = (m_PresentQueueFamily != graphicsFamily);
    ...
    if (result != VK_SUCCESS) {
        WE_LOG_CRITICAL(
            "RendererInit",
            std::string("vkCreateSwapchainKHR failed (VkResult=") +
                std::to_string(static_cast<int32_t>(result)) +
                ", extent=" + std::to_string(extent.width) + "x" + std::to_string(extent.height) +
                ", format=" + std::to_string(static_cast<int32_t>(surfaceFormat.format)) + ")");
    }
