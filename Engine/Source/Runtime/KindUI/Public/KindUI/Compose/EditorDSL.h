...
public:
    ...
    void Property(std::string label, KindUIWidgetPtr valueWidget, std::function<void(FieldConfig&)> config = nullptr) {
        Field(std::move(label), std::move(valueWidget), std::move(config));
    }
