...
RHIResult<RHIDescriptorSetHandle> DX12Device::AllocateDescriptorSet(const DescriptorSetAllocateDesc& desc) {
    if (!m_Pools.count(static_cast<uint64_t>(desc.pool)) || !m_SetLayouts.count(static_cast<uint64_t>(desc.layout))) {
...
void DX12Device::UpdateDescriptorSets(std::span<const WriteDescriptorSet>) {}
...
RHIResult<RHIGraphicsPipelineHandle> DX12Device::CreateGraphicsPipeline(const GraphicsPipelineDesc& desc) {
    ...
    if (haveShaders && m_Device) {
        ...
        if (SUCCEEDED(D3D12SerializeRootSignature(&rsDesc, D3D_ROOT_SIGNATURE_VERSION_1, &signature, &error))
            && SUCCEEDED(m_Device->CreateRootSignature(
                   0, signature->GetBufferPointer(), signature->GetBufferSize(), IID_PPV_ARGS(&rootSig)))) {
            ...
            if (SUCCEEDED(m_Device->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&pso)))) {
                pipeline.rootSignature = std::move(rootSig);
