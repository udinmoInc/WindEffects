...
public:
    [[nodiscard]] static TextResult<FontAsset> LoadFromFile(const std::filesystem::path& path);
    [[nodiscard]] static TextResult<FontAsset> LoadFromMemory(std::span<const std::byte> data);
};

class TEXT_API FontAssetWriter {
public:
    [[nodiscard]] static TextResult<void> WriteToFile(
