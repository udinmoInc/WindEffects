    ...
    private async Task<List<string>> ResolveTransitiveDependencies(string sourceFile, HashSet<string> visited)
    {
        ...
        foreach (var include in directIncludes)
        {
            ...
            if (!string.IsNullOrEmpty(resolvedPath) && File.Exists(resolvedPath))
            {
                ...
                if (resolvedPath.EndsWith(".h", StringComparison.OrdinalIgnoreCase)
                    || resolvedPath.EndsWith(".hpp", StringComparison.OrdinalIgnoreCase)
                    || resolvedPath.EndsWith(".hxx", StringComparison.OrdinalIgnoreCase)
                    || resolvedPath.EndsWith(".inl", StringComparison.OrdinalIgnoreCase))
                {
