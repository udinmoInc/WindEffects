...
/// <summary>
/// Clang compiler implementation (cross-platform).
/// </summary>
public class ClangCompiler : ICompiler
{
    ...
    public ClangCompiler(TargetPlatform platform = TargetPlatform.Unknown)
    {
        _platform = platform;
    ...
    public async Task<string> DetectVersionAsync()
    {
        try
        {
            ...
            // Parse version from output (e.g., "clang version 14.0.0")
            var versionMatch = Regex.Match(output, @"clang version (\d+\.\d+\.\d+)");
        ...
        {
            Log.Warning(ex, "Failed to detect Clang version");
            return "Unknown";
        }
    }
    ...
    public async Task<CompilationResult> CompileAsync(CompilerOptions options)
    {
        ...
        try
        {
            ...
            Log.Information("Compiling {SourceFile} with Clang", options.SourceFile);
    ...
    private string BuildCompileArguments(CompilerOptions options)
    {
        ...
        if (_platform == TargetPlatform.Windows)
        {
            ...
            args.Add("-fms-runtime-library=dynamic");
        }
    ...
    private string FindExecutable()
    {
        ...
        {
            TargetPlatform.Windows => "clang++.exe",
            _ => "clang++"
        };
    ...
    private List<CompilerDiagnostic> ParseDiagnostics(string output, string sourceFile)
    {
        ...
        // Clang error format: filename:line:column: error/warning: message
