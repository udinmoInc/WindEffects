...
void PlaceActorsCategory::PaintEmptyState(PaintContext& context, const Rect& bounds, const std::string& message) {
    const float fontSize = ResolveMetric(MetricToken::TextSizeSmall);
    const float x = bounds.x + ActorsPanelLayout::ContentPadH();
    const float y = bounds.y + (bounds.height - fontSize) * 0.5f;
    context.DrawText(message, Point{ x, y }, ResolveColor(ColorToken::TextSecondary), fontSize);
}
                                      ...
                                      bool showChevron)
{
    ...
    if (showChevron) {
        ...
        cursorX += chevronSize + ResolveMetric(MetricToken::Space1) * uiScale;
    }
    ...
    if (isFavoritesSection) {
        ...
        cursorX += iconDraw + ResolveMetric(MetricToken::Space1) * uiScale;
